...
LocLin.Ini<-function(data.list,n,nmax,grid.l,grids,r){
    ...
    #hmu.cv = h.select(t,y,method = "aicc")
    # hmu.cv = h.select(t,y)
...
oneReplicate_LOC = function(seedJ){
    set.seed(seedJ + repID * 300)
    source("./oneReplicate/oneRep-LOC.R")
    #    res = FPCA(DatanewObsCol$Ly, DatanewObsCol$Lt, list(methodSelectK = 8))
    IniVal = Initial(r.set, ini.method, data.list.new, n, nmax, grid.l, grids, iter.num = 100, basis.EM = "poly", sig.EM)
        ## Basis method ns and poly, poly is the cubic B-spline
...
oneReplicateWrap_LOC = function(seedJ){
    try({
        eval = oneReplicate_LOC(seedJ)
    })
    return(eval)
}

LOCInit = function(obsCol,  splineObj, r, sigmaSq){
    ...
    IniVal = Initial(r, ini.method = "loc", data.list.new, n, nmax, grid.l, grids)
...
MFPCA_LOC = function(obsCol, r.set, splineObj){
    tmin = 0
    tmax = 1
    nmax = max(table(obsCol[ ,1]))
    L1 = min(as.numeric(obsCol[ ,3]))
    ...
    IniVal = Initial(r.set, ini.method = "loc", data.list.new, n, nmax, grid.l, grids)
    sig2hat = IniVal[[1]]
    covmatrix.ini<-IniVal[[2]]
    eigenf.ini<-IniVal[[3]]
    eigenv.ini<-IniVal[[4]]
    ...
    fList_est = Generate_fList(grids, eigenfest)   ### using grid.l here 
    model = list(eigenFunctions = fList_est)
    model = c(model, list(tmin = tmin, tmax = tmax, SFinal = XInit,
                          sigmaSq = sig2hat, numPCA = nrow(eigenfest), numElem = 1, elemLevels = "1"))
