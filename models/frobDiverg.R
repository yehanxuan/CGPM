createMFPCAObjfrobDiverg = function(dataF, splineObj, obsSigma = 1){
...
#    ## 重新加了Omega在这里
 #   Omega = Omega / max(Omega)
    ...
    optObj = new(frobDiverg, obsY, bMatLarge, obsSigma, obsIDCount)
    optObj$set_penaltyMatrix(Gamma)
    return(optObj)
}


MFPCA_EstimatefrobDiverg = function(obsData, splineObj, 
                               optRank, mu,
                               controlList1 = NULL,
                               controlList2 = NULL, 
                               cvParam,
                               SInit = NULL, sigmaSq, eigenfList = NULL){
    ...
    trainObj = createMFPCAObjfrobDiverg(obsData, splineObj, sigmaSq)
    ...
    estimate = MFPCA_SecondfrobDiverg(trainObj, optRank, controlList2, SInit, splineObj, eigenfList)
...
MFPCA_SecondfrobDiverg = function(optObj, optRank, controlList, SInit, splineObj , eigenfList ){
    SEstimate = SInit
    iter = 0
    flag = TRUE
    gap = 1
...
#        SEstimate = MFPCA_SecondCore(optObj, optRank, controlList, SEstimate)
        #params = c(0.9, 1)
        #SEstimate = MFPCA_SecondCore(trainObj, 5 ,controlList2, SInit)
#        U = SEstimate[[1]]
#        W = SEstimate[[2]]
#        S = U %*% W %*% t(U)
#        optObj$updateSigmaSq(S)  #
        ...
        for (j in 1:L2){
            ...
            try({
                for (cf in 1:nFold){
                    ...
                    ErrorObj = createMFPCAObjfrobDiverg(obsCol, splineObj, obsSigma = model$sigmaSq)
...
MFPCA_frobDiverg = function(obsCol, order, nknots, rankSeq, lambdaSeq, controlList1, controlList2,
  ...
  #  obsCol = subtractMeanCurve(meanModel, obsCol)
    ...
    model = MFPCA_EstimatefrobDiverg(obsCol, splineObj, select$opt_rank, select$opt_mu,
                                     controlList1, controlList2, cvParam, SInit, sigmaSq, eigenfList)
    if (is.null(eigenfList)){
      loss = NULL
    } else {
    ...
    return(list("opt_lambda" = opt_lambda, "opt_rank"= opt_rank, "loss" = loss, "model" = model, "meanModel" = meanModel))
}
...
oneReplicateWrap_frobDiverg = function(seedJ){
    try({
        eval = oneReplicate_frobDiverg(seedJ)
    })
    return(eval)
}
