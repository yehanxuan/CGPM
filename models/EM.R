...
MFPCA_EM = function(obsCol, M.set, r.set, sig.EM, eigenfList = NULL,InitType = NULL){
  ...
  select = EM_selection(newObsCol, M.set, r.set, basis.EM = "poly", nFold = 10, eigenfList = eigenfList, InitType = InitType)
...
oneReplicate_EM = function(seedJ){
    ...
    return(list("loss" = loss, "result2" = M_opt, "rank" = r_opt, "converge" = converge, "runTime" = runTime, 
                "sysTime" = sysTime, "eigenValues" = eigenv.ini))
}
