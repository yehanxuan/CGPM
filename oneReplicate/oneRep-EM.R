...
if (DataType == "cai"){
    ...
    M.set = seq(8, 15, by = 1)
    pcaCompNum = 8 
    pcaKappa = c(1, 0.66, 0.52, 0.07, 9.47e-3, 1.28e-3, 1.74e-4, 2.35e-5, 3.18e-6, 4.3e-7 )
    pcaKappaSqrt = sqrt(pcaKappa)
...
#    pcaTrans = qr.Q(qr(pcaTrans))
    ...
    pcaKappaSqrt = sqrt( c(3,2, 1, 0.7, 0.25, 0.1, 0.07, 1e-3, 5e-4, 5e-5) )
    #pcaKappaSqrt = sqrt(c(1, 0.66, 0.52, 0.43, 0.38, 0.25, 0.07, 9.47e-3, 1.28e-3, 1.17e-4, 2.35e-5, 3.18e-6))
    # scoreType = "Gaussian"
    obsCol = get_allObs_NonUnif(eigenfList, samplesize, pcaKappaSqrt, noiseSigma = 1/2, obsNumLower = 7, obsNumUpper = 14,
                                scoreType)
    #lambdaSeq = exp(seq(-22, -13, length.out = 5))
    #nKnots = 20
    M.set = seq(10, 18, by = 2)
    r.set = 6
    ...
    pcaKappaSqrt = sqrt(pcaKappa)
    eigenfList = get_eigenfunList(pcaTrans, fExpNum, splitP)
    obsCol = get_allObs(eigenfList, samplesize, pcaKappaSqrt, noiseSigma = 1, obsNumLower, obsNumUpper, scoreType, noiseType)
